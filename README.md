...
- **Оценивает лид:** 🔥 горячий, 🟡 тёплый, ❄️ холодный. Оценка прозрачная — менеджер видит, за что начислены баллы (сроки, бюджет относительно цены, оставлен ли телефон).
- **Создаёт лид в Битрикс24:** имя, телефон, Telegram, сумма сделки, анкета, причины оценки и переписка с клиентом.
- **Присылает карточку менеджеру в Telegram** со ссылкой на лид в Битрикс24. Команда `/leads` — последние заявки.
...
```
bot.py        — диалог с клиентом, приём телефона, передача лида, напоминания
qualifier.py  — ИИ-квалификация: промпт, извлечение анкеты, подбор курса, оценка лида
crm.py        — Битрикс24: создание лида, карточка для менеджера
